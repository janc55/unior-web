export const careerDetails = {
    slug: "derecho", // Único identificador para la ruta
    title: "Derecho",
    description: "Forma profesionales con un profundo conocimiento en leyes y justicia, capaces de interpretar, aplicar y defender el marco jurídico en diversas áreas del derecho, tanto a nivel nacional como internacional.",
    duration: "4.5 años",
    degree: "Licenciatura",
    modality: "Presencial (Modalidad Anual)",
    credits: 350,
    coordinator: "Dr. Samuel Tapia",
    accreditation: "R.M. Nro 095/99",
    image: "/carreras/der.webp?height=600&width=800",
    video: "/carreras/der.mp4",
    youtubeId: "afraJA5_D-E?si=_gAPjS4dxqU9JYlx",
    icon: "⚖️",
    curriculum: [
      { year: "Primer Año", courses: ["Economía Política", "Fundamentos de la Filosofía y Materialismo Dialéctico", "Historia del Derecho y Derecho Romano", "Introducción al Derecho", "Derecho Civil I: Personas", "Sociología General y Materialismo Histórico", "Técnicas de Investigación Social", "Criminología y DD.HH.", "Idioma Quechua", "Optativa Básica"] },
      { year: "Segundo Año", courses: ["Derecho Administrativo y Autonómico", "Pluralismo Jurídico y Justicia Comunitaria", "Derecho Procesal Orgánico y MARC´s", "Teoría del Estado y del Derecho", "Derecho Civil II: Obligaciones", "Derecho del Trabajo y Segurida Social", "Pensamiento Crítico e Historia de Bolivia", "Derecho Penal I", "Idioma Inglés", "Materia Optativa"] },
      { year: "Tercer Año", courses: ["Defensa Legal del Estado","Derecho Agroambiental y Procedimiento Agrario", "Derecho Constitucional y Procedimientos Constitucionales", "Derecho de las Familias, Niña, Niño Y Adolescente", "Derecho Civil III: Contratos", "Derecho Minero e Hidrocarburos", "Derecho Internacional Público", "Derecho Penal II", "Derecho Comercial y Empresarial"] },
      { year: "Cuarto Año", courses: ["Derecho Financiero Tributario y Aduanero", "Derecho Procesal Penal", "Derecho Procesal Civil", "Práctica Forense Civil", "Derecho Civil IV: Sucesiones", "Medicina Legal", "Derecho Internacional Privado", "Práctica Forense Penal", "Práctica en Procedimientos Especiales"] },
      { year: "Taller de Grado", courses: ["Electiva I", "Taller de Grado"] }
  ],
    jobOpportunities: [
      "Abogado Litigante",
      "Consultor Legal",
      "Asesor Jurídico",
      "Juez o Fiscal",
      "Notario",
      "Defensor Público",
      "Especialista en Derecho Corporativo",
      "Docente Universitario en Derecho",
    ],
    studentTestimonials: [
      {
        name: "María González",
        position: "Abogada en Estudio Jurídico González & Asociados",
        quote: "Estudiar Derecho en la UNIOR me permitió desarrollar habilidades analíticas y argumentativas que son fundamentales en mi trabajo diario. Los profesores son expertos en sus áreas y siempre están dispuestos a ayudar a los estudiantes a alcanzar su máximo potencial.",
        image: "/carreras/aud.webp?height=100&width=100"
      },
      {
        name: "Carlos Ramírez",
        position: "Asesor Legal en Empresa CARDOMAR",
        quote: "La carrera de Derecho me brindó una formación integral en leyes y justicia, así como la oportunidad de realizar prácticas profesionales en empresas y estudios jurídicos reconocidos. Estoy muy agradecido por la calidad educativa y el apoyo recibido durante mi formación.",
        image: "/carreras/aud.webp?height=100&width=100"
      },
    ],
    brochureImages: [
      "/carreras/brochure/DER-1.webp",
      "/carreras/brochure/DER-2.webp"
    ],
    brochurePdf: "https://drive.google.com/uc?id=1mUaD9BwFKWbz4KZ94QytZQFlS1HrB82b&export=download"
  };
  